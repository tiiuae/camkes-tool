...
class DtbMatchQuery(Query):
    ...
    def xlat_regs(cls, cells):
        ...
        while True:
            ...
            addr = cls.xlat_reg(addr, size, cells['buses'])
    ...
    def resolve_fdt_node(cls, node):
        ...
        # from the parents
        resolved['buses'] = cls.resolve_buses(node)
        ...
        if 'reg' in resolved:
            resolved['buses'] = cls.resolve_translations(resolved['buses'])
            cls.xlat_regs(resolved)
